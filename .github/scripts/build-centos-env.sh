...
perl -0pi -e 's{&& cd rnnoise}{&& cd rnnoise && sed -i "s|#include .os_support.h.|#define OPUS_CLEAR(dst,n) __builtin_memset(dst,0,(n)*sizeof(*(dst)))|" src/vec_neon.h}g' "$dockerfile_path"
